...
pub fn development_testnet_config() -> Result<ChainSpec, String> {
	...
	properties.insert("tokenSymbol".into(), "SRS".into());
...
pub fn latest_dawn_testnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		ChainType::Live,
		// SECRET="..."
		// ./target/debug/subkey inspect "$SECRET//sunrise//root"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//oracle"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//1//validator"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//1//babe"
		// ./target/debug/subkey --ed25519 inspect "$SECRET//sunrise//1//grandpa"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//2//validator"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//2//babe"
		// ./target/debug/subkey --ed25519 inspect "$SECRET//sunrise//2//grandpa"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//3//validator"
		// ./target/debug/subkey --sr25519 inspect "$SECRET//sunrise//3//babe"
		// ./target/debug/subkey --ed25519 inspect "$SECRET//sunrise//3//grandpa"
		move || {
			dawn_genesis(
				wasm_binary,
				vec![
...
fn testnet_genesis(
	...
	use dawn_runtime::{
		dollar, get_all_module_accounts, SunriseOracleConfig, AirDropConfig, Balance, BalancesConfig, BandOracleConfig,
		...
		OperatorMembershipSunriseConfig, OperatorMembershipBandConfig, OrmlNFTConfig, ParachainInfoConfig,
		RenVmBridgeConfig, StakingPoolConfig, SudoConfig, SystemConfig, TechnicalCommitteeMembershipConfig,
		TokensConfig, VestingConfig, SRS, SUSD, DOT, LDOT, RENBTC, XBTC,
	};
	#[cfg(feature = "std")]
	...
	let initial_balance: u128 = 1_000_000 * dollar(SRS);
	let initial_staking: u128 = 100_000 * dollar(SRS);

	let evm_genesis_accounts = evm_genesis();

	let balances = initial_authorities
		.iter()
		.map(|x| (x.0.clone(), initial_staking + dollar(SRS))) // bit more for fee
	...
	dawn_runtime::GenesisConfig {
		...
		pallet_membership_Instance5: OperatorMembershipSunriseConfig {
			members: vec![root_key.clone()],
			phantom: Default::default(),
		},
		...
		srs_pallet_shy_treasury: ShyTreasuryConfig {
		...
		srs_pallet_shy_engine: ShyEngineConfig {
			collaterals_params: vec![
				(
					...
					10_000_000 * dollar(SUSD),                           // maximum debit value in aUSD (cap)
				),
				(
					XBTC,
					Some(FixedU128::zero()),
		...
		srs_pallet_airdrop: AirDropConfig {
			airdrop_accounts: vec![],
		},
		orml_oracle_Instance1: SunriseOracleConfig {
		...
		srs_pallet_evm: EVMConfig {
			accounts: evm_genesis_accounts,
		},
		srs_pallet_staking_pool: StakingPoolConfig {
			staking_pool_params: srs_pallet_staking_pool::Params {
...
fn dawn_genesis(
	...
	use dawn_runtime::{
		cent, dollar, get_all_module_accounts, SunriseOracleConfig, AirDropConfig, AirDropCurrencyId, Balance,
		...
		ShyCouncilMembershipConfig, IndicesConfig, NativeTokenExistentialDeposit, OperatorMembershipSunriseConfig,
		OperatorMembershipBandConfig, OrmlNFTConfig, ParachainInfoConfig, RenVmBridgeConfig, StakingPoolConfig,
		SudoConfig, SystemConfig, TechnicalCommitteeMembershipConfig, TokensConfig, VestingConfig, SRS, SUSD, DOT,
		LDOT, RENBTC, XBTC,
	};
	#[cfg(feature = "std")]
	use sp_std::collections::btree_map::BTreeMap;
	...
	dawn_runtime::GenesisConfig {
		...
		srs_pallet_airdrop: AirDropConfig {
			airdrop_accounts: {
				let srs_airdrop_accounts_json =
					&include_bytes!("../../../../../resources/dawn-airdrop-SRS.json")[..];
				let srs_airdrop_accounts: Vec<(AccountId, Balance)> =
					serde_json::from_slice(srs_airdrop_accounts_json).unwrap();
				...
				srs_airdrop_accounts
					.iter()
					.map(|(account_id, srs_amount)| (account_id.clone(), AirDropCurrencyId::SRS, *srs_amount))
					.chain(
		...
		orml_nft: OrmlNFTConfig {
			tokens: {
				...
				let nft_airdrop: Vec<(
					AccountId,
					Vec<u8>,
					srs_pallet_nft::ClassData,
					Vec<(Vec<u8>, srs_pallet_nft::TokenData, Vec<AccountId>)>,

...
//! SHYs has debit, cancel all active auctions module, when debits and gaps are
...
use sp_std::prelude::*;
use srs_pallet_support::{AuctionManager, SHYTreasury, EmergencyShutdown, PriceProvider, Ratio};
...
pub mod module {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + srs_pallet_loans::Config {
		...
		/// SHY treasury to escrow collateral assets after settlement
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn open_collateral_refund(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			...
			// Ensure all debits of SHYs have been settled, and all collateral auction has
			// been done or canceled. Settle all collaterals type SHYs which have debit,
			...
			for currency_id in collateral_currency_ids {
				...
				// there's on debit in SHY
				ensure!(
					<srs_pallet_loans::Module<T>>::total_positions(currency_id).debit.is_zero(),
		...
		pub fn refund_collaterals(
			...
			let refund_ratio: Ratio = <T as Config>::SHYTreasury::get_debit_proportion(amount);
			let collateral_currency_ids = T::CollateralCurrencyIds::get();

			// burn caller's stable currency by SHY treasury
			<T as Config>::SHYTreasury::burn_debit(&who, amount)?;

			let mut refund_assets: Vec<(CurrencyId, Balance)> = vec![];
			// refund collaterals to caller by SHY treasury
			for currency_id in collateral_currency_ids {
				let refund_amount =
					refund_ratio.saturating_mul_int(<T as Config>::SHYTreasury::get_total_collaterals(currency_id));

				if !refund_amount.is_zero() {
					<T as Config>::SHYTreasury::withdraw_collateral(&who, currency_id, refund_amount)?;

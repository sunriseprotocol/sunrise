...
//! ## Overview
//!
//! SHY Treasury manages the accumulated interest and bad debts generated by
//! SHYs, and handle excessive surplus or debits timely in order to keep the
...
use srs_pallet_support::{AuctionManager, SHYTreasury, SHYTreasuryExtended, DEXManager, Ratio};
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		/// The Currency for managing assets related to SHY
		...
		#[pallet::constant]
		/// The SHY treasury's module id, keep surplus and collateral assets
		/// from liquidation.
	...
	#[pallet::error]
	pub enum Error<T> {
		/// The collateral amount of SHY treasury is not enough
		CollateralNotEnough,
		/// The surplus pool of SHY treasury is not enough
		SurplusPoolNotEnough,
		/// debit pool overflow
		DebitPoolOverflow,
		/// The debit pool of SHY treasury is not enough
		DebitPoolNotEnough,
	}
	...
	/// Current total debit value of system. It's not same as debit in SHY
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn auction_collateral(
			...
			T::UpdateOrigin::ensure_origin(origin)?;
			<Self as SHYTreasuryExtended<T::AccountId>>::create_collateral_auctions(
				currency_id,
				amount,
				target,
				Self::account_id(),
...
impl<T: Config> Pallet<T> {
	/// Get account of shy treasury module.
	...
	/// Get total collateral amount of shy treasury module.
...
impl<T: Config> SHYTreasury<T::AccountId> for Pallet<T> {
...
impl<T: Config> SHYTreasuryExtended<T::AccountId> for Pallet<T> {

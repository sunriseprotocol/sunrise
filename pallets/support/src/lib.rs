...
pub mod slip;
pub use slip::{
	SlipProtocol, NomineesProvider, OnCommission, OnNewEra, PolkadotBridge, PolkadotBridgeCall, PolkadotBridgeState,
...
/// An abstraction of shy treasury for Honzon Protocol.
pub trait SHYTreasury<AccountId> {
	type Balance;
	type CurrencyId;

	/// get surplus amount of shy treasury
	fn get_surplus_pool() -> Self::Balance;

	/// get debit amount of shy treasury
	fn get_debit_pool() -> Self::Balance;

	/// get collateral assets amount of shy treasury
	...
	/// issue debit for shy treasury
	fn on_system_debit(amount: Self::Balance) -> DispatchResult;

	/// issue surplus(stable currency) for shy treasury
...
pub trait ExchangeIncentives<AccountId, CurrencyId, Balance> {
	fn do_deposit_exchange_share(who: &AccountId, lp_currency_id: CurrencyId, amount: Balance) -> DispatchResult;
	fn do_withdraw_exchange_share(who: &AccountId, lp_currency_id: CurrencyId, amount: Balance) -> DispatchResult;
}
...
//TODO #[derive(Encode, Decode, Eq, PartialEq, Copy, Clone, RuntimeDebug)]

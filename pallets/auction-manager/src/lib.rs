...
use srs_primitives::{AuctionId, Balance, CurrencyId};
use sp_runtime::{
	offchain::{
...
use sp_std::prelude::*;
use srs_pallet_support::{AuctionManager, SHYTreasury, SHYTreasuryExtended, DEXManager, EmergencyShutdown, PriceProvider, Rate};
...
pub mod module {
	...
	pub trait Config: frame_system::Config + SendTransactionTypes<Call<Self>> {
		...
		/// SHY treasury to escrow assets related to auction
		type SHYTreasury: SHYTreasuryExtended<Self::AccountId, Balance = Balance, CurrencyId = CurrencyId>;
...
impl<T: Config> Pallet<T> {
	...
	fn cancel_debit_auction(id: AuctionId, debit_auction: DebitAuctionItem<T::BlockNumber>) -> DispatchResult {
		...
		if let Some((bidder, _)) = Self::get_last_bid(id) {
			...
			T::SHYTreasury::issue_debit(&bidder, debit_auction.fix, false)?;
	...
	fn cancel_collateral_auction(
		...
		// refund remain collateral to refund recipient from SHY treasury
		T::SHYTreasury::withdraw_collateral(
		...
		if let Some((bidder, bid_price)) = last_bid {
			// refund stable token to the bidder
			T::SHYTreasury::issue_debit(&bidder, bid_price, false)?;
	...
	pub fn collateral_auction_bid_handler(
		...
		<CollateralAuctions<T>>::try_mutate_exists(
			...
			|collateral_auction| -> sp_std::result::Result<T::BlockNumber, DispatchError> {
				...
				// transfer remain payment from new bidder to SHY treasury
				T::SHYTreasury::deposit_surplus(&new_bidder, payment)?;

				// if collateral auction will be in reverse stage, refund collateral to it's
				// origin from auction SHY treasury
		...
		<DebitAuctions<T>>::try_mutate_exists(
			...
			|debit_auction| -> sp_std::result::Result<T::BlockNumber, DispatchError> {
				...
				if let Some(last_bidder) = last_bidder {
					...
					// there's no bid before, transfer stablecoin to SHY treasury
					T::SHYTreasury::deposit_surplus(&new_bidder, debit_auction.fix)?;
				}
	...
	fn collateral_auction_end_handler(
		...
		if let Some((bidder, bid_price)) = winner {
					...
					.unwrap_or_default()
			{
				...
				if let Ok(stable_amount) = T::SHYTreasury::swap_exact_collateral_in_auction_to_stable(
					collateral_auction.currency_id,
					collateral_auction.amount,
					Zero::zero(),
					None,
					...
					let _ = T::SHYTreasury::issue_debit(&bidder, bid_price, false);
					...
					if collateral_auction.in_reverse_stage(stable_amount) {
						...
						let _ = T::SHYTreasury::issue_debit(&collateral_auction.refund_recipient, refund_amount, false);
					}
			...
			if should_deal {
				// transfer collateral to winner from SHY treasury, it shouldn't fail and affect
				// the process. but even it failed, just the winner did not get the amount. it
				// can be fixed by treasury council.
				let _ = T::SHYTreasury::withdraw_collateral(
					&bidder,
					collateral_auction.currency_id,
					collateral_auction.amount,
				);
	...
	fn surplus_auction_end_handler(
		...
		if let Some((bidder, bid_price)) = winner {
			// deposit unbacked stable token to winner by SHY treasury, it shouldn't fail
			// and affect the process. but even it failed, just the winner did not get the
			// amount. it can be fixed by treasury council.
			let _ = T::SHYTreasury::issue_debit(&bidder, surplus_auction.amount, false);

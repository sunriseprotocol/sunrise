...
//! ## Overview
//!
//! The entry of the Honzon protocol for users, user can manipulate their SHY
//! position to loan/payback, and can also authorize others to manage the their
//! SHY under specific collateral type.
//!
...
use srs_primitives::{Amount, CurrencyId};
use sp_runtime::{
	traits::{StaticLookup, Zero},
	DispatchResult,
};
use srs_pallet_support::EmergencyShutdown;
...
pub mod module {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + srs_pallet_shy_engine::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		///   collateral currency into SHY, negative means withdraw collateral
		///   currency from SHY.
		...
		///   SHY according to to the debit adjustment.
		...
		pub fn adjust_loan(
			...
			<srs_pallet_shy_engine::Module<T>>::adjust_position(&who, currency_id, collateral_adjustment, debit_adjustment)?;
			Ok(().into())
		}

		/// Transfer the whole SHY of `from` under `currency_id` to caller's SHY
		...
		pub fn transfer_loan_from(
			...
			<srs_pallet_loans::Module<T>>::transfer_loan(&from, &to, currency_id)?;
			Ok(().into())
		}
		...
		#[pallet::weight(<T as Config>::WeightInfo::unauthorize_all(<T as srs_pallet_shy_engine::Config>::CollateralCurrencyIds::get().len() as u32))]
		#[transactional]

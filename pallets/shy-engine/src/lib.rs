...
//! ## Overview
//!
//! The core pallet of SHY protocol. SHY engine is responsible for handling the
//! internal processes about lending, including liquidation, settlement and risk
//! management.
...
use srs_pallet_support::{
	SHYTreasury, SHYTreasuryExtended, DEXManager, EmergencyShutdown, ExchangeRate, Price, PriceProvider, Rate, Ratio,
	RiskManager,
};
...
pub const OFFCHAIN_WORKER_DATA: &[u8] = b"srsla/shy-engine/data/";
pub const OFFCHAIN_WORKER_LOCK: &[u8] = b"srsla/shy-engine/lock/";
pub const OFFCHAIN_WORKER_MAX_ITERATIONS: &[u8] = b"srsla/shy-engine/max-iterations/";
pub const LOCK_DURATION: u64 = 100;
pub const DEFAULT_MAX_ITERATIONS: u32 = 1000;

pub type LoansOf<T> = srs_pallet_loans::Module<T>;

/// Risk management params
#[derive(Encode, Decode, Clone, RuntimeDebug, PartialEq, Eq, Default)]
pub struct RiskManagementParams {
	/// Maximum total debit value generated from it, when reach the hard
	/// cap, SHY's owner cannot issue more stablecoin under the collateral
	/// type.
	...
	/// SHY under this collateral type is below the liquidation ratio, this
	/// SHY is unsafe and can be liquidated. `None` value means not set
	pub liquidation_ratio: Option<Ratio>,

	/// Liquidation penalty rate, when liquidation occurs,
	/// SHY will be deducted an additional penalty base on the product of
	...
	/// of SHY so that the current collateral ratio is lower than the
...
pub enum LiquidationStrategy {
	/// Liquidation SHY's collateral by create collateral auction
	Auction,
	/// Liquidation SHY's collateral by swap with DEX
	Exchange,
}
...
pub mod module {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + srs_pallet_loans::Config + SendTransactionTypes<Call<Self>> {
		...
		#[pallet::constant]
		/// The default liquidation ratio for all collateral types of SHY
		...
		#[pallet::constant]
		/// The default liquidation penalty rate when liquidate unsafe SHY
		...
		#[pallet::constant]
		/// The max slippage allowed when liquidate an unsafe SHY by swap with
		/// DEX
		type MaxSlippageSwapWithDEX: Get<Ratio>;

		/// The SHY treasury to maintain bad debts and surplus generated by SHYs
		type SHYTreasury: SHYTreasuryExtended<Self::AccountId, Balance = Balance, CurrencyId = CurrencyId>;

		/// The price source of all types of currencies related to SHY
	...
	pub enum Error<T> {
		...
		/// The SHY must be unsafe to be liquidated
		MustBeUnsafe,
		/// Invalid collateral type
		InvalidCollateralType,
		/// Remain debit value in SHY below the dust amount
		RemainDebitValueTooSmall,
		/// Feed price is invalid
		InvalidFeedPrice,
		/// No debit value in SHY so that it cannot be settled
		NoDebitValue,
	...
	pub enum Event<T: Config> {
		/// Liquidate the unsafe SHY. \[collateral_type, owner,
		/// collateral_amount, bad_debt_value, liquidation_strategy\]
		LiquidateUnsafeSHY(CurrencyId, T::AccountId, Balance, Balance, LiquidationStrategy),
		/// Settle the SHY has debit. [collateral_type, owner]
		SettleSHYInDebit(CurrencyId, T::AccountId),
	...
	impl<T: Config> Hooks<T::BlockNumber> for Pallet<T> {
			...
			if !T::EmergencyShutdown::is_shutdown() {
				for currency_id in T::CollateralCurrencyIds::get() {
					...
					if !stability_fee_rate.is_zero() && !total_debits.is_zero() {
						...
						if <T as Config>::SHYTreasury::on_system_surplus(issued_stable_coin_balance).is_ok() {
		...
		/// Runs after every block. Start offchain worker to check SHY and
		/// submit unsigned tx to trigger liquidation or settlement.
		fn offchain_worker(now: T::BlockNumber) {
			if let Err(e) = Self::_offchain_worker() {
				log::info!(
					target: "shy-engine offchain worker",
	...
	impl<T: Config> Pallet<T> {
		/// Liquidate unsafe SHY
		///
		/// The dispatch origin of this call must be _None_.
		///
		/// - `currency_id`: SHY's collateral type.
		/// - `who`: SHY's owner.
		...
		pub fn liquidate(
			...
			Self::liquidate_unsafe_shy(who, currency_id)?;
			Ok(().into())
		}

		/// Settle SHY has debit after system shutdown
		///
		/// The dispatch origin of this call must be _None_.
		///
		/// - `currency_id`: SHY's collateral type.
		...
		pub fn settle(
			...
			Self::settle_shy_has_debit(who, currency_id)?;
			Ok(().into())
		}

		/// Update global parameters related to risk management of SHY
		///
		...
		/// Update parameters related to risk management of SHY under specific
		/// collateral type
		///
		/// The dispatch origin of this call must be `UpdateOrigin`.
		///
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			match call {
				Call::liquidate(currency_id, who) => {
					...
					if !Self::is_shy_unsafe(*currency_id, collateral, debit) || T::EmergencyShutdown::is_shutdown() {
						return InvalidTransaction::Stale.into();
					}

					ValidTransaction::with_tag_prefix("SHYEngineOffchainWorker")
...
impl<T: Config> Pallet<T> {
	...
	fn _offchain_worker() -> Result<(), OffchainErr> {
		...
		let mut map_iterator = <srs_pallet_loans::Positions<T> as IterableStorageDoubleMapExtended<_, _, _>>::iter_prefix(
			currency_id,
			max_iterations,
			start_key.clone(),
		);
		...
		while let Some((who, Position { collateral, debit })) = map_iterator.next() {
			if !is_shutdown && Self::is_shy_unsafe(currency_id, collateral, debit) {
				// liquidate unsafe SHYs before emergency shutdown occurs
				Self::submit_unsigned_liquidation_tx(currency_id, who);
			} else if is_shutdown && !debit.is_zero() {
				// settle SHYs with debit after emergency shutdown occurs.
	...
	pub fn is_shy_unsafe(currency_id: CurrencyId, collateral: Balance, debit: Balance) -> bool {
	...
	// settle shy has debit when emergency shutdown
	pub fn settle_shy_has_debit(who: T::AccountId, currency_id: CurrencyId) -> DispatchResult {
		let Position { collateral, debit } = <LoansOf<T>>::positions(currency_id, &who);
		ensure!(!debit.is_zero(), Error::<T>::NoDebitValue);

		// confiscate collateral in shy to shy treasury
		// and decrease SHY's debit to zero
		...
		Self::deposit_event(Event::SettleSHYInDebit(currency_id, who));
		Ok(())
	}

	// liquidate unsafe shy
	pub fn liquidate_unsafe_shy(who: T::AccountId, currency_id: CurrencyId) -> DispatchResult {
		let Position { collateral, debit } = <LoansOf<T>>::positions(currency_id, &who);

		// ensure the shy is unsafe
		ensure!(
			Self::is_shy_unsafe(currency_id, collateral, debit),
			Error::<T>::MustBeUnsafe
		);

		// confiscate all collateral and debit of unsafe shy to shy treasury
		...
		let liquidation_strategy = (|| -> Result<LiquidationStrategy, DispatchError> {
			...
			if let Ok(actual_supply_collateral) =
				<T as Config>::SHYTreasury::swap_collateral_not_in_auction_with_exact_stable(
					currency_id,
					target_stable_amount,
					collateral,
					Some(T::MaxSlippageSwapWithDEX::get()),
				) {
				// refund remain collateral to SHY owner
				...
				<T as Config>::SHYTreasury::withdraw_collateral(&who, currency_id, refund_collateral_amount)?;

				return Ok(LiquidationStrategy::Exchange);
			}

			// create collateral auctions by shy treasury
			<T as Config>::SHYTreasury::create_collateral_auctions(
				currency_id,
				collateral,
				target_stable_amount,
				who.clone(),
		...
		Self::deposit_event(Event::LiquidateUnsafeSHY(
			currency_id,
			who,
			collateral,
			bad_debt_value,

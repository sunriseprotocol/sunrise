...
pub mod module {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn redeem(
			...
			match strategy {
				RedeemStrategy::Immediately => {
					T::Slip::redeem_by_free_unbonded(&who, amount)?;
				}
				RedeemStrategy::Target(target_era) => {
					T::Slip::redeem_by_claim_unbonding(&who, amount, target_era)?;
				}
				RedeemStrategy::WaitForUnbonding => {
					T::Slip::redeem_by_unbond(&who, amount)?;
				}
			}
			Ok(().into())
		}
		...
		pub fn withdraw_redemption(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			let who = ensure_signed(origin)?;
			T::Slip::withdraw_redemption(&who)?;
			Ok(().into())
		}
	}
}

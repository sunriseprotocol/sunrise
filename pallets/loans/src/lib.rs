...
//! ## Overview
//!
//! Loans module manages SHY's collateral assets and the debits backed by these
//! assets.
...
use srs_pallet_support::{SHYTreasury, RiskManager};
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		/// Risk manager is used to limit the debit size of SHY
		type RiskManager: RiskManager<Self::AccountId, CurrencyId, Balance, Balance>;

		/// SHY treasury for issuing/burning stable currency adjust debit value
		/// adjustment
		type SHYTreasury: SHYTreasury<Self::AccountId, Balance = Balance, CurrencyId = CurrencyId>;

		/// The loan's module id, keep all collaterals of SHYs.
		#[pallet::constant]
	...
	pub enum Event<T: Config> {
		...
		/// Confiscate SHY's collateral assets and eliminate its debit. \[owner,
